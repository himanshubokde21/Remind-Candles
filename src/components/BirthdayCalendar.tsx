...
type BirthdayMapKey = `${number}-${number}`;
...
export const BirthdayCalendar = () => {
  ...
  const birthdayMap = new Map<BirthdayMapKey, Birthday[]>();
  birthdays.forEach(birthday => {
    const date = new Date(birthday.birthDate);
    const key = `${date.getMonth()}-${date.getDate()}` as BirthdayMapKey;
  ...
  const hasBirthdayOnDate = (day: number, month: number): boolean => {
    return birthdayMap.has(`${month}-${day}` as BirthdayMapKey);
  };

  const getBirthdaysOnDate = (day: number, month: number): Birthday[] => {
    return birthdayMap.get(`${month}-${day}` as BirthdayMapKey) || [];
  };
